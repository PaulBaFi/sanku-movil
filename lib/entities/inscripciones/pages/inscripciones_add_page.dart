...
class _InscripcionesAddPageState extends State<InscripcionesAddPage> {
  // Controladores - CAMBIO: solo fecha de inscripción
  final fechaInscripcionController = TextEditingController();
  ...
  int? numeroSesionesPaquete; // NUEVO
  ...
  void initState() {
    ...
    fechaInscripcionController.text = DateFormat(
      'yyyy-MM-dd',
    ).format(DateTime.now());
  }
  ...
  Future<void> _saveInscripcion() async {
    final fechaInscripcion = fechaInscripcionController.text.trim();
    ...
    if (fechaInscripcion.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Fecha de inscripción es obligatoria")),
      );
      return;
    }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              _generarQR
                  ? "Inscripción, $numeroSesionesPaquete sesiones y pago creados correctamente"
                  : "Inscripción y $numeroSesionesPaquete sesiones creadas correctamente",
            ),
        ...
        child: Column(
          ...
          children: [
              ...
              onChanged: (value) {
                setState(() {
                  ...
                  if (index != -1) {
                    ...
                    numeroSesionesPaquete =
                        paquetes[index]['numero_sesiones'] ?? 0; // NUEVO
                  } else {
                    selectedPaqueteId = null;
                    precioPaqueteSeleccionado = null;
                    numeroSesionesPaquete = null;
                  }
                });
              },
            ),

            // NUEVO: Mostrar info del paquete
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '• Precio: S/ ${precioPaqueteSeleccionado!.toStringAsFixed(2)}',
                      style: TextStyle(fontSize: 14, color: Colors.blue.shade700),
                    ),
                    Text(
                      '• Sesiones: $numeroSesionesPaquete (se crearán automáticamente)',
                      style: TextStyle(fontSize: 14, color: Colors.blue.shade700),
                    ),
                  ],
                ),
            ...
            WidgetFieldDatePicker(
              controller: fechaInscripcionController,
